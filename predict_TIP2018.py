...
def get_args():
    ...
    parser.add_argument('--model', '-m', default='./saved_models/facades3/lastest.pth',
                        # '/data1/liul/model/generatorsole7_100.pth',
                        metavar='FILE',
...
if __name__ == "__main__":
    ...
    for i, fn in enumerate(im_files):
        ...
        img = Image.open(os.path.join(root, in_files, fn)).convert('RGB')
        # pdb.set_trace()
        [w, h] = img.size
        ...
        imgcom = img
        imggt = Image.open(os.path.join(root, in_files2, im_files2[i])).convert('RGB')
        ...
        print('PSNR: %f, SSIM: %f,  MSE: %f' % (p, s, mse))
